...
class Game {
    ...
    monstersStart(){
        ...
        this.timescoreId = setInterval((this.timescore.updateTime()), 1000);
        if (this.isPlayerIntersectWithMonster()){
            console.log('GameOver');
            this.lives.substractLife();
            console.log(this.lives)
            if(this.lives.playerLives == 0){
                alert(`Game Over! You scored: ${this.timescoreId}sec!`);
                clearInterval(this.gameIntervalId);
                clrearInterval(this.timescoreId);
